...
export function BarterDetailPage() {
 ...
 <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-600 hover:text-gold-600 mb-6 transition-colors">
 <ArrowLeft size={16} /> Back
 </button>

 <div className="card mb-6 border border-gold-500/20">
 <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
 <div className="grid h-24 w-24 shrink-0 place-items-center rounded-xl bg-gold-600/10 text-3xl font-bold text-gold-600 border border-gold-500/20 shadow-glow">
 ...
 <h1 className="font-display text-3xl font-bold text-gray-900">{targetUser?.name}</h1>
 <p className="text-gray-600 mt-1">{targetUser?.college} • {targetUser?.department} • Sem {targetUser?.semester}</p>
 {targetUser?.bio && <p className="text-sm text-gray-700 mt-2">{targetUser.bio}</p>}
 </div>
 {!isNew && (
 ...
 <h3 className="text-xs font-bold uppercase tracking-wider text-gold-600 mb-2">They can teach</h3>
 <p className="text-gray-800">{teaches}</p>
 </div>
 <div className="rounded-xl bg-charcoal p-5 border border-line">
 <h3 className="text-xs font-bold uppercase tracking-wider text-sky mb-2">They want to learn</h3>
 <p className="text-gray-800">{wants}</p>
 </div>
 </div>
 </div>
 ...
 <span className="block text-sm font-bold text-gray-600 mb-2">Describe what you want to barter and how you plan to exchange skills</span>
 <textarea 
 ...
 <p className="text-gray-800 whitespace-pre-wrap">{description || "No description provided."}</p>
 </div>
