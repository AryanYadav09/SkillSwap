          ...
          <NavLink
            ...
            className={({ isActive }) =>
              [
                ...
                isActive
                  ? "bg-gold-500/10 text-gold-400 border border-gold-500/20 shadow-glow"
                  : "text-gray-400 hover:bg-charcoal hover:text-gold-300",
...
function ProfileDropdown({ onNavigate }) {
  ...
  return (
    <div className="relative">
      <button 
        className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-bold text-gray-400 hover:bg-charcoal hover:text-gold-300 transition-all duration-300"
              ...
              {profileLinks.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-2 text-sm font-medium text-gray-300 hover:bg-obsidian hover:text-gold-400"
                  >
                    <Icon size={16} />
                    {item.label}
                  </Link>
    ...
    <div className="page-band min-h-screen bg-obsidian">
      ...
      <header className="sticky top-0 z-40 border-b border-line bg-charcoal/80 backdrop-blur-xl shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              className="btn border border-gold-500/30 bg-gold-500/10 px-2 sm:px-3 text-gold-400 hover:bg-gold-500/20 shadow-none relative" 
      ...
      {notificationsOpen ? (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setNotificationsOpen(false)}>
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <h2 className="font-display text-xl font-bold text-gray-100 flex items-center gap-2">
                <Bell size={20} className="text-gold-400" />
                Notifications
                {unreadCount > 0 && <span className="text-xs text-gold-400 bg-gold-500/10 rounded-full px-2 py-0.5">{unreadCount}</span>}
              </h2>
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/80 backdrop-blur-md">
      <div className="mx-4 w-full max-w-sm rounded-2xl border border-gold-500/20 bg-charcoal p-8 text-center shadow-glow-lg">
        ...
        <p className="text-lg font-bold text-gray-100">Incoming Video Call</p>
        <p className="mt-2 text-sm text-gray-400">
          <span className="font-bold text-gold-400">{call.callerName}</span> is calling you
        </p>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-gold-500/10 text-gold-400">
                    <Icon size={16} />
                  </div>
      ...
      <div className={`w-full transition-all duration-300 ${isRegister ? 'max-w-4xl' : 'max-w-md'}`}>
        <section className="glass rounded-2xl p-6 sm:p-10 shadow-glow-lg">
          <div className="mb-10 text-center">
            ...
            <p className="mt-3 text-sm text-gray-400">Trade what you know. Learn what you need.</p>
          </div>
          <Outlet />
        </section>
      </div>
